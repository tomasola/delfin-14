...
export const Scanner: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    // Verificar duplicados (usando el estado calculado o re-verificando por seguridad)
    if (isDuplicate) {
      alert(`⛔ DUPLICADO DETECTADO\n\nLa referencia "${formData.reference}" ya existe en el historial.\n\nNo se ha guardado el registro.`);
      return;
    }
      ...
      <div className="flex flex-col h-full overflow-y-auto p-4 pb-24">
        ...
        <div className="fixed bottom-[70px] left-0 right-0 p-4 bg-slate-900/90 backdrop-blur border-t border-slate-800 flex gap-3">
          ...
          <Button
            className="flex-[2]"
            onClick={handleSave}
            disabled={isDuplicate} // Opcional: Deshabilitar visualmente también
